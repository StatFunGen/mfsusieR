...
- [ ] 1.2 Add the chosen implementation. Refresh the docstring at
      `R/ibss_methods.R:228-240` to remove the "V[l] = 1 always"
      claim.
- [ ] 1.3 Add a one-line comment to
...
- [ ] 2.1 Add a paragraph to `mfsusie()` return-value roxygen for
      `pip`, naming the V-based filter and the `prior_tol`
      argument.
- [ ] 2.2 NEWS.md entry under the next release header.

## 3. Test

- [ ] 3.1 New regression test (`tests/testthat/test_v_filter_pip.R`)
      on a null-only fixture asserting the V-filter behavior is
      deterministic and documented.

## 4. Validate

- [ ] 4.1 `openspec validate address-effective-V-consumer-coherence`
      from `inst/`.
