...
- [ ] 1.1 Add `validate_init.mf_individual` (or extend
...
- [ ] 1.2 New unit test exercising the validator on a corrupted
      `model_init` fixture.

## 2. A-3. Post-hook S3 dispatch cleanup

- [ ] 2.1 In `post_loglik_prior_hook.mf_individual`
...
- [ ] 2.2 Drop the unused `moments = get_post(model, l)`
      argument.
- [ ] 2.3 Confirm the existing test suite still passes.

## 3. B-5. CS purity / coverage parity smoke test

- [ ] 3.1 New `tests/testthat/test_cs_parity_fsusier.R` that
...
- [ ] 4.1 Add `combine_outcome_lbfs.default` in
...
- [ ] 4.2 New unit test asserting the helpful error message.

## 5. C-8. L_greedy expansion regression test

- [ ] 5.1 New `tests/testthat/test_l_greedy_expansion.R` fitting
...
- [ ] 6.1 `openspec validate audit-2026-04-30-followups` from
      `inst/`.
- [ ] 6.2 Full `devtools::test()` clean.
