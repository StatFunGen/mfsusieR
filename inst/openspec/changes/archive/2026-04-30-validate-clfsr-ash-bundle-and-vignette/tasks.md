...
- [x] 1.1 `tests/testthat/test_smash_ash_fidelity.R`. Three
      `test_that` blocks:
...
- [x] 1.2 `tests/testthat/test_ti_uniform_fidelity.R`. Two
      `test_that` blocks:
      ...
      - Same at T_m = 128.
- [x] 1.3 Both test files start with
...
- [x] 2.1 Append to `tests/testthat/test_clfsr.R`: a test that
...
- [x] 3.1 Append a section
      ...
      - Forwarding `nullweight` via `...` (1 chunk, 3 lines of
        prose).
- [x] 3.2 Build the vignette locally and confirm it renders
      without errors / warnings.

## 4. Validate

- [x] 4.1 `openspec validate validate-clfsr-ash-bundle-and-vignette`
      from `inst/`.
- [x] 4.2 Full `devtools::test()` clean.
- [x] 4.3 `R CMD check --as-cran` returns 0 errors, 0 warnings on
      the new files.
