...
- [x] 2.1 Add `mf_smash_lw(noisy_signal, noise_level, n.shifts = 50,
      ...
      lengths.
- [x] 2.2 Add `flavor = c("ash", "smashr")` arg to
...
- [x] 2.3 Add `flavor = c("ash", "smashr")` arg to `mf_post_smooth`.
      Forward to `.post_smooth_smash` and from there to
      `univariate_smash_regression`.
- [x] 2.4 Move the `requireNamespace("smashr")` gate in
      `mf_post_smooth` so it fires only when `flavor = "smashr"`.
- [x] 2.5 New test `tests/testthat/test_smash_lw.R`: shape;
...
- [x] 3.1 Add `scaling = c("per_scale", "uniform")` arg to
      ...
      - `"uniform"`: skip Y scaling, single ash with nullweight = 3,
        rescale by `1 / csd_X`.
- [x] 3.2 Forward `scaling` from `.post_smooth_ti` and
      `mf_post_smooth(method = "TI", scaling = ...)`.
- [x] 3.3 New test `tests/testthat/test_ti_uniform.R`: shape;
...
- [x] 4.1 `openspec validate add-clfsr-smash-lw-ti-uniform` from
      `inst/`.
