# Locks the contract for save_mu_method = c("complete", "aggregated", "lead")
# and the mf_thin() post-fit helper.
#
# Invariants enforced:
#   1. complete is the default and produces fits with attr "complete".
#   2. aggregated reduces fit$mu / fit$mu2 to 1 x T per (l, m) and adds
...
#   4. coef.mfsusie complete == aggregated numerically (1e-12).
#   5. coef.mfsusie complete != lead in general (cheap-coef, biased).
#   6. mf_post_smooth scalewise complete == aggregated numerically (1e-12).
#   7. mf_thin(complete, "aggregated") == mfsusie(..., "aggregated").
...
test_that("aggregated shrinks mu/mu2 to 1 x T and adds coef_wavelet", {
  .timed("aggregated shape", {
    obj <- build_toy_fit(save_mu_method = "aggregated")
    expect_equal(attr(obj$fit, "save_mu_method"), "aggregated")
...
test_that("coef.mfsusie aggregated is numerically equivalent to complete", {
  .timed("coef aggregated equiv", {
    obj_full <- build_toy_fit(save_mu_method = "complete")
    obj_ac   <- build_toy_fit(save_mu_method = "aggregated")
...
test_that("mf_post_smooth scalewise aggregated equals complete (1e-12)", {
  .timed("post_smooth aggregated equiv", {
    obj_full <- build_toy_fit(save_mu_method = "complete")
    obj_ac   <- build_toy_fit(save_mu_method = "aggregated")
...
test_that("mf_thin(complete, 'aggregated') matches a fresh aggregated fit", {
  .timed("mf_thin aggregated equiv", {
    obj_full <- build_toy_fit(save_mu_method = "complete")
    obj_ac   <- build_toy_fit(save_mu_method = "aggregated")
    thinned  <- mf_thin(obj_full$fit, method = "aggregated")
    expect_equal(attr(thinned, "save_mu_method"), "aggregated")
