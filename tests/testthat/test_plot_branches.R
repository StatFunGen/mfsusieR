...
build_multi_outcome_fit <- function(seed = 5L) {
  ...
  mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, X, list(Y1, Y2), L = 3, max_iter = 30, verbose = FALSE)
}
...
build_three_outcome_fit <- function(seed = 7L) {
  ...
  mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, X, list(Y1, Y2, Y3), L = 3, max_iter = 30, verbose = FALSE)
}
...
test_that(".resolve_facet picks 'stack' for K = 2 with disjoint affected masks", {
  ...
  fit <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, Y, X, pos = seq_len(T_m), L = 4, verbose = FALSE)
  fit_h <- mf_post_smooth(fit, method = "HMM")
...
test_that("mfsusie_plot draws the dot-plot panel on a scalar outcome", {
  ...
  fit <- mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, X, list(matrix(Y_scalar, n, 1L), Y_func),
