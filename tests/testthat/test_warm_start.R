...
test_that("capping max_iter then resuming via model_init reaches convergence", {
  sim <- build_warm_sim()
  fit_partial <- fsusie(sim$Y, sim$X, L = 5,
                        max_iter = 2,
                        verbose = FALSE)
  expect_false(isTRUE(fit_partial$converged))

  fit_resumed <- fsusie(sim$Y, sim$X, L = 5,
...
test_that("model_init with smaller L is expanded to the requested L", {
  sim <- build_warm_sim()
  fit_small <- fsusie(sim$Y, sim$X, L = 3,
                      verbose = FALSE)
  fit_grown <- fsusie(sim$Y, sim$X, L = 5,
...
test_that("model_init with larger L than requested errors cleanly", {
  ...
  fit_cold <- fsusie(sim$Y, sim$X, L = 5,
                     verbose = FALSE)
  expect_error(
    fsusie(sim$Y, sim$X, L = 3, model_init = fit_cold,
           verbose = FALSE),
