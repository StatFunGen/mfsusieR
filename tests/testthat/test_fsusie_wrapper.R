...
test_that("fsusie() runs end-to-end on a single-modality fixture", {
  ...
  fit <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, Y, X, L = 3, max_iter = 30, verbose = FALSE)
...
test_that("fsusie(Y, X, pos) produces a bit-identical fit to mfsusie(X, list(Y), list(pos))", {
  ...
  fit_w <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, Y, X, pos = pos, L = 3, max_iter = 30, verbose = FALSE)
  fit_m <- mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, X, list(Y), pos = list(pos), L = 3, max_iter = 30, verbose = FALSE)
...
test_that("fsusie() errors when the user passes a multi-modality argument", {
  ...
  expect_error(
    fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, Y, X, cross_outcome_prior = list()),
...
test_that("fsusie() errors on non-numeric Y", {
  X <- matrix(rnorm(30), nrow = 5)
  Y_bad <- letters[1:6]
  expect_error(fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, Y_bad, X), "must be a numeric matrix")
})
