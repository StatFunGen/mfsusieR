...
make_M1 <- function(L = 5, n = 80, p = 25, T_m = 64) {
  ...
  fit <- mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, X, Y, L = L, verbose = FALSE)
...
make_M2 <- function(L = 5, n = 80, p = 25, T_per = c(32L, 32L)) {
  ...
  Y <- lapply(T_per, function(T_m)
    X %*% matrix(rep(beta, T_m), nrow = p) +
      matrix(rnorm(n * T_m, sd = 0.5), n))
  fit <- mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, X, Y, L = L,
...
test_that("mfsusie_plot scalar-outcome path (T_m = 1)", {
  ...
  fit <- mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, X, Y_scalar, L = 3, verbose = FALSE)
...
test_that("mfsusie_plot_lfsr renders >2 outcome grid with leftover cell", {
  ...
  Y <- lapply(c(32L, 32L, 32L), function(T_m)
    X %*% matrix(rep(beta, T_m), nrow = p) +
      matrix(rnorm(n * T_m, sd = 0.5), n))
  fit <- mfsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, X, Y, L = 3,
                 prior_variance_scope = "per_outcome",
                 verbose = FALSE)
