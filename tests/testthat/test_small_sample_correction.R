...
test_that("small_sample_correction = FALSE leaves the fit bit-identical", {
  ...
  fit_default <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, sim$Y, sim$X, L = 5,
                        verbose = FALSE)
  fit_explicit <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, sim$Y, sim$X, L = 5,
...
test_that("small_sample_correction = TRUE runs to convergence on small n", {
  sim <- build_small_n_sim()
  fit_johnson <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, sim$Y, sim$X, L = 5,
...
test_that("Johnson-t reduces PIP at known-null variants vs Wakefield", {
  ...
  fit_wake <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, sim$Y, sim$X, L = 5,
                     small_sample_correction = FALSE,
                     verbose = FALSE)
  fit_john <- fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, sim$Y, sim$X, L = 5,
                     small_sample_correction = TRUE,
                     verbose = FALSE)
...
test_that("small_sample_correction rejects non-logical input", {
  sim <- build_small_n_sim()
  expect_error(
    fsusie(wavelet_qnorm = FALSE, wavelet_standardize = FALSE, max_iter = 100, sim$Y, sim$X, L = 5,
