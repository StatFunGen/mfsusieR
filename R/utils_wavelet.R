...
col_scale <- function(x,
  ...
  if (add_attr) {
    ...
    # (which has happened above). fsusieR computes the same
    # quantity via a two-step `(a + b - a) / b` expression that
    # introduces ULP-level rounding noise; we compute the
...
#'
#' This is a behaviour-preserving simplification of fsusieR's
#' `interpol_mat` plus the position-rescaling step that fsusieR
#' performs separately in `remap_data`. The original chain rescaled
#' the wavethresh `gridt` (in `[0, 1]`) to a `[0, length(grid)]`
#' integer scale, then back to position units; we collapse the two
#' rescalings into one linear interpolation that produces the same
#' numbers in fewer passes over `pos`.
#'
